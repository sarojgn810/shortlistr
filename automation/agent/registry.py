...
_BUILTINS = [
    ...
    Tool("shortlistr.skip", "Skip / discard a job from the pipeline.",
         WRITE, {"job_id": {"type": "string", "required": True}}, None),
    Tool("shortlistr.whoami", "Return this user's profile snapshot (name, targeting, CV highlights).",
         READ, {}, None),
    Tool("shortlistr.prep", "Generate prep materials (cover letter + interview guide + CV PDF) for a job.",
         WRITE, {"job_id": {"type": "string", "required": True}}, None),
