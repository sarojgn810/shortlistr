...
def _default_profile() -> dict[str, Any]:
    ...
    return {
        ...
        # A fresh profile points at the recommended free cloud model rather than
        # a local tag. Local AI is still supported, but it is now the advanced
        # path: the models a laptop can run are the ones that struggle most with
        # structured evaluation, and that should not be a new user's first
        # impression of the product.
        "llm": {"provider": "auto", "model": RECOMMENDED_FREE_MODEL,
...
# The free path new users are pointed at: Google AI Studio issues a key without
# a credit card, and gemini-2.0-flash is the cheapest capable model on it. Named
# once here so onboarding, Connections and the profile default cannot drift.
RECOMMENDED_FREE_PROVIDER = "gemini"
RECOMMENDED_FREE_MODEL = "gemini-2.0-flash"
...
def save_profile_from_ui(body: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        llm_model = coerce_cloud_model(provider, llm_model)
    except Exception:
        pass
...
# Providers the user picked on purpose. Turning Local AI on should not silently
# move someone off the cloud model they chose and are paying for.
_CLOUD_PROVIDERS = ("anthropic", "openai", "gemini", "grok", "groq")


def adopt_local_ai_provider(model: str = "") -> dict[str, Any]:
    """Point the profile at Local AI after the user enables it in Connections.

    Enabling Local AI pulled the model and wrote a status file, but never
    touched `llm.provider`. A fresh profile reads back as "none" — see
    get_profile_for_ui — and get_llm() returns None for "none", so every
    evaluation fell through to keyword scoring and the UI showed "Basic score"
    on a machine with a working local model. Choosing a model has to be the
    thing that makes it get used.

    A deliberately chosen cloud provider is left alone; "auto" already prefers a
    ready local model, so only the tag is recorded there.
    """
    profile = get_profile_for_ui()
    current = (profile.get("llm_provider") or "none").lower()
    if current in _CLOUD_PROVIDERS:
        return profile

    payload = {**profile}
    if current != "auto":
        payload["llm_provider"] = "auto"
    if model:
    ...
    if not profile.get("exists"):
        # Nothing to merge onto yet — onboarding will fill the rest in.
        payload.setdefault("name", "Demo User")
        payload.setdefault("email", "demo@example.com")
        payload.setdefault("target_titles", profile.get("target_titles") or [])
        if not payload["target_titles"]:
            return profile

    try:
        return save_profile_from_ui(payload)
    except ValueError:
        # An incomplete profile is onboarding's problem, not a reason to fail
        # the Local AI setup the user just asked for.
        return profile
