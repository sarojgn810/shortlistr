...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = "", max_tokens: int = 1024,
        ...
        try:
            ...
            if json_mode:
                # Gemini's equivalent of the JSON grammar the other providers
                # get. Without it the evaluation prompt comes back as prose and
                # the parser reports "No JSON object in LLM response" — the same
                # failure that made every card read "Basic score".
                kwargs["response_mime_type"] = "application/json"
            try:
                config = types.GenerateContentConfig(**kwargs)
            except TypeError:
                # Older google-genai builds do not know the field.
