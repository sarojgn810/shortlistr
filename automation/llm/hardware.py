...
def setup_guide(system: dict[str, Any] | None = None, *, model_id: str | None = None) -> list[dict[str, str]]:
    ...
    steps.extend(
        [
            ...
            {
                "title": "You’re done",
                "body": "When status says Ready, scoring and cover letters use Local AI — "
                        "unless you have also saved an API key, which Auto prefers. No account "
                        "needed either way, and expect thinner analysis than a cloud model gives.",
            },
        ]
    )
    return steps
