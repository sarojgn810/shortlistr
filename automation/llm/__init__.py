...
def _looks_like_ollama_model(model: str) -> bool:
    """Ollama tags use ``name:tag`` (e.g. ``qwen2.5:0.5b``); cloud IDs do not."""
    m = (model or "").strip()
    return bool(m) and ":" in m and "/" not in m.split(":", 1)[0]
...
def _default_cloud_model(provider: str) -> str:
    ...
    if provider == "openai":
        from .openai_llm import DEFAULT_MODEL

        return DEFAULT_MODEL
    if provider == "anthropic":
        from .anthropic import DEFAULT_MODEL
...
def coerce_cloud_model(provider: str, model: str) -> str:
    """Drop leftover Ollama tags when the active provider is a cloud API.

    Saving Local AI then switching to Groq used to leave ``qwen2.5:0.5b`` in
    profile.yml — Groq then 404s on every chat/eval call.
    """
    p = (provider or "").lower().strip()
    m = (model or "").strip()
    if p not in ("groq", "openai", "anthropic", "gemini", "grok"):
        return m
    if not m or _looks_like_ollama_model(m):
        return _default_cloud_model(p)
    return m


def _persist_coerced_model(provider: str, before: str, after: str) -> None:
    """Write the healed model back to profile.yml + in-memory config.

    Otherwise status/UI keep showing the dead Ollama tag and the next process
    boot can race a stale cache before coerce runs.
    """
    ...
    try:
        ...
        _cfg.LLM_CONFIG["model"] = after
    except Exception:
        pass
    try:
        import config as _cfg

        path = _cfg.PROFILE_PATH
    except Exception:
        return
    ...
    for line in lines:
        ...
        if in_llm and stripped.startswith("model:"):
            ...
            out.append(f'{indent}model: "{after}"\n')
            changed = True
            continue
        out.append(line)
    if not changed:
    ...
    try:
        with open(path, "w", encoding="utf-8") as f:
            f.writelines(out)
        logger.warning(
            "Replaced incompatible LLM model %r with %r for provider %s (saved to profile)",
            before,
            after,
            provider,
        )
    except OSError as e:
        logger.warning("Could not persist coerced LLM model: %s", e)


def _build_cloud(provider: str, api_key: str, model: str) -> Optional[LLMProvider]:
    raw = (model or "").strip()
    model = coerce_cloud_model(provider, model)
    if model != raw:
        _persist_coerced_model(provider, raw, model)
    if provider == "anthropic":
...
def _resolve_auto(api_key: str, model: str, ollama_url: str) -> tuple[Optional[LLMProvider], str]:
    ...
    Order: cloud key (auto-detect provider) → Local AI ready → none (heuristics).

    A key comes first because it is the deliberate act. Local AI used to win,
    which meant a machine with a leftover model quietly overrode the key the
    user had just pasted — and the models a laptop can run are the ones that
    struggle most with this workload. Somebody running Local AI on purpose
    selects `ollama` explicitly and is not routed through here at all.
    """
    from .local_ai import RECOMMENDED_MODEL, is_local_ready, usable_local_model
    ...
    # Neither the configured tag nor the recommended one is pulled — but the
    # user may well have a different model already. Checking only those two sent
    # a machine holding gemma3:12b to heuristics with no key.
    adopted = usable_local_model(ollama_url)
    if adopted:
        logger.info("Local AI: using already-installed model %s", adopted)
        return OllamaProvider(model=adopted, base_url=ollama_url), "ollama"
