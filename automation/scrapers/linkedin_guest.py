...
def _request_page(
    ...
    *,
    ...
    `from_cache` is not just a speed signal — the caller skips its pacing sleep
    on a hit, because there is no request to be gentle about.
    """
    ...
    # Imported here: sources/__init__ builds the registry, which imports adapters
    # that import this module.
    from sources.fetcher import read_cached_text, text_cache_key, write_cached_text

    key = text_cache_key(SEARCH_URL, params)
    cached = read_cached_text(key)
    ...
    write_cached_text(key, response.text)
    ...
    for title, location, remote in build_searches():
        for page in range(max(1, max_pages)):
            try:
                ...
                served_from_cache = served_from_cache and from_cache
            ...
            # Pace only between real requests. Sleeping after a cache hit would
            # spend the whole delay budget having sent nothing.
            if page + 1 < max_pages and not from_cache:
