...
# Setup has exactly two supported entry points: `make start` for a first run,
# and the dashboard afterwards. Doctor must not send anyone back to hand-copying
# templates or editing .env — those paths still exist but they are not the ones
# we support, and a hint is where users pick up habits.
FIRST_RUN = "Run: make start   (installs, seeds, and opens onboarding)"
...
def check_cv() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "Onboarding asks for your résumé and writes cv.md"],
    }
...
def check_profile() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "Onboarding writes it from your résumé — no hand-editing"],
    }
...
def check_portals() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "It seeds a starter watchlist; edit it in Connections"],
    }
...
def check_applications() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "It seeds the tracker automatically"],
    }
...
def check_python_deps() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "It installs Python deps, Chromium, and dashboard deps"],
    }
...
def check_config() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "Onboarding writes a valid config/profile.yml"],
    }
...
def check_httpx() -> dict:
    ...
    return {
        ...
        "fix": [FIRST_RUN, "httpx ships in automation/requirements.txt"],
    }
    ...
    try:
        ...
        if st["configured"]:
            return {
                ...
                "fix": ["Re-check the key in the dashboard → Connections",
                        "Evaluation still works without it, using the heuristic scorer"],
            }
        return {
            "pass": False,
            "label": "LLM provider is 'none' — template eval only",
            "fix": ["Add a key (or set up Local AI) in the dashboard → Connections",
                    "Optional: evaluation falls back to the heuristic scorer"],
        }
    except Exception as e:
