...
#
# Unverified Gmail alerts (no live posting confirmed yet) are held the same way.
# Missing verification on a Gmail row defaults to unverified; other sources default
# to confirmed so existing ATS/portal rows keep showing.
RELEVANT_ONLY = (
    "AND COALESCE(json_extract(j.metadata_json, '$.discovery_relevance'), 'relevant') "
    "!= 'off_target' "
    "AND NOT ("
    "  LOWER(COALESCE(j.source, '')) = 'gmail' "
    "  AND COALESCE(json_extract(j.metadata_json, '$.verification'), 'unverified') "
    "      = 'unverified'"
    ")"
)
