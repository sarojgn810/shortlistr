...
def enrich_job_dict(job: dict[str, Any], *, slim: bool = False) -> dict[str, Any]:
    ...
    try:
        from processors.job_display import apply_structure_to_job

        structured = apply_structure_to_job(
            {
                **out,
                "experience": out.get("experience") or "",
            }
        )
        out["title"] = structured.get("title") or out["title"]
        out["company"] = structured.get("company") or out["company"]
        out["location"] = structured.get("location") or out["location"]
        if structured.get("experience") and not out.get("experience"):
