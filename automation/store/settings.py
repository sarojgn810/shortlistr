...
DEFAULT_AUTOMATION: dict[str, Any] = {
    ...
    "auto_evaluate": True,
    # Pre-eval fit gate (0–100 discovery fit_score). Legacy installs stored ≤5
    # from an old 0–5 UI; get_automation_settings remaps those to 40.
...
def get_automation_settings(tenant_id: str = "default") -> dict[str, Any]:
    out = _get_json(tenant_id, "automation", DEFAULT_AUTOMATION)
    # Old default was 4.0 on a mistaken 0–5 scale; the setting is a 0–100 fit gate.
    try:
        raw = float(out.get("auto_evaluate_min_score") or 40)
    except (TypeError, ValueError):
        raw = 40.0
    if 0 < raw <= 5:
        raw = 40.0
    out["auto_evaluate_min_score"] = max(0, min(100, raw))
    return out
