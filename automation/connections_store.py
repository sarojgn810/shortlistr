...
_SECRET_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    # The IMAP app password, for anyone not on Gmail. Same underlying secret as
    # the Gmail SMTP app password — one mailbox credential, whichever provider.
    "SHORTLISTR_EMAIL_PASSWORD": ("SHORTLISTR_EMAIL_PASSWORD", "GMAIL_APP_PASSWORD"),
    "TELEGRAM_BOT_TOKEN": ("TELEGRAM_BOT_TOKEN",),
    "APIFY_TOKEN": ("APIFY_TOKEN",),
    "SHORTLISTR_EMAIL_VERIFY_API_KEY": ("SHORTLISTR_EMAIL_VERIFY_API_KEY", "HUNTER_API_KEY"),
    "SHORTLISTR_SERPER_API_KEY": ("SHORTLISTR_SERPER_API_KEY", "SERPER_API_KEY"),
...
def _page_reader_status(data: dict[str, Any] | None = None) -> dict[str, Any]:
    """Optional page-reader boost for empty SPA / blocked careers pages."""
    data = data if data is not None else _load_yaml()
    sources = data.get("sources") if isinstance(data.get("sources"), dict) else {}
    ...
    if "enabled" in pr:
        ...
        # Legacy key from the short-lived Agent Reach experiment.
    ...
    return {
        ...
        "ready": enabled,
        "hint": (
            "Recovers empty careers / JD pages before falling back to Playwright."
            if enabled
            else "Turn on to recover empty or blocked careers pages (no API key)."
        ),
    }


def _mailbox_status(data: dict[str, Any] | None = None) -> dict[str, Any]:
    """Which inbox job-alert ingest and outcome tracking will read.

    Gmail stays the zero-config path when its OAuth token is present. Everyone
    else gives an address and an app-specific password here; the server is
    inferred for the common providers and typed in otherwise. The password goes
    to the keychain like every other credential and is never returned.
    """
    cfg = (data or _load_yaml()).get("mailbox")
    cfg = cfg if isinstance(cfg, dict) else {}
    host = str(cfg.get("imap_host") or "").strip()
    user = str(cfg.get("imap_user") or "").strip()
    if not host and "@" in user:
        try:
            from mail import KNOWN_IMAP_HOSTS

            host = KNOWN_IMAP_HOSTS.get(user.rsplit("@", 1)[1].lower(), "")
        except Exception:
            host = ""
    gmail_ready = bool(_gmail_oauth_status().get("token_present"))
    imap_ready = bool(host and user and _secret_set("SHORTLISTR_EMAIL_PASSWORD"))
    return {
        "provider": str(cfg.get("provider") or "auto"),
        "imap_host": host,
        "imap_user": user,
        "imap_port": int(cfg.get("imap_port") or 993),
        "password_set": _secret_set("SHORTLISTR_EMAIL_PASSWORD"),
        "gmail_ready": gmail_ready,
        "imap_ready": imap_ready,
        # What ingestion will actually use, so the UI can say so plainly rather
        # than leaving a non-Gmail user guessing why they get no email jobs.
        "active": "gmail" if gmail_ready else ("imap" if imap_ready else None),
    }
...
def get_connections_for_ui() -> dict[str, Any]:
    ...
    return {
        ...
        "mailbox": _mailbox_status(data),
        "telegram": {
            "token_set": _secret_set("TELEGRAM_BOT_TOKEN"),
        },
        "apify": _apify_status(data),
        "email_verify": {
            "api_key_set": _secret_set("SHORTLISTR_EMAIL_VERIFY_API_KEY"),
            "provider": str(
        ...
        "serper": {
            "api_key_set": _secret_set("SHORTLISTR_SERPER_API_KEY"),
        },
        "adzuna": {
            "app_id_set": _secret_set("ADZUNA_APP_ID"),
            "app_key_set": _secret_set("ADZUNA_APP_KEY"),
            "ready": _secret_set("ADZUNA_APP_ID") and _secret_set("ADZUNA_APP_KEY"),
        },
        "google_cse": {
            "api_key_set": _secret_set("GOOGLE_CSE_API_KEY"),
...
def save_connections_from_ui(body: dict[str, Any]) -> dict[str, Any]:
    ...
    # Mailbox (IMAP) — for anyone not on Gmail. Address and server are ordinary
    # config; the app password goes to the keychain like every other credential.
    mailbox_keys = ("mailbox_provider", "mailbox_imap_host", "mailbox_imap_user",
                    "mailbox_imap_port")
    if any(k in body for k in mailbox_keys):
        mb = dict(data.get("mailbox") or {}) if isinstance(data.get("mailbox"), dict) else {}
        if body.get("mailbox_provider") is not None:
            mb["provider"] = str(body["mailbox_provider"]).strip().lower() or "auto"
        if body.get("mailbox_imap_host") is not None:
            mb["imap_host"] = str(body["mailbox_imap_host"]).strip()
        if body.get("mailbox_imap_user") is not None:
            mb["imap_user"] = str(body["mailbox_imap_user"]).strip()
        if body.get("mailbox_imap_port") is not None:
            try:
                mb["imap_port"] = int(body["mailbox_imap_port"])
            except (TypeError, ValueError):
                mb["imap_port"] = 993
        data["mailbox"] = mb
        changed_profile = True
    if "adzuna_app_id" in body:
        _write_secret("ADZUNA_APP_ID", body.get("adzuna_app_id"))
    if "adzuna_app_key" in body:
        _write_secret("ADZUNA_APP_KEY", body.get("adzuna_app_key"))
    if "mailbox_password" in body:
        _write_secret("SHORTLISTR_EMAIL_PASSWORD", body.get("mailbox_password"))
    ...
    if "email_verify_api_key" in body:
        _write_secret("SHORTLISTR_EMAIL_VERIFY_API_KEY", body.get("email_verify_api_key"))
    if "serper_api_key" in body:
        _write_secret("SHORTLISTR_SERPER_API_KEY", body.get("serper_api_key"))
    ...
    if "github_token" in body:
        _write_secret("SHORTLISTR_GITHUB_TOKEN", body.get("github_token"))
    ...
    if "page_reader_enabled" in body and body["page_reader_enabled"] is not None:
        sources = data.get("sources") if isinstance(data.get("sources"), dict) else {}
        sources = dict(sources)
        pr = sources.get("page_reader") if isinstance(sources.get("page_reader"), dict) else {}
        pr = dict(pr)
        pr["enabled"] = bool(body["page_reader_enabled"])
        sources["page_reader"] = pr
        # Drop legacy key so Connections and is_enabled() stay in sync.
        sources.pop("agent_reach", None)
        data["sources"] = sources
        changed_profile = True
...
def _reject_non_api_key(value: Any) -> None:
    """Catch the credential people actually copy off the Google console.

    Google Cloud → Credentials lists OAuth client IDs above API keys, and the
    client ID is the one that looks like a key. Saved as an API key it produces
    a 400 from customsearch on the first search, hours later, with nothing
    connecting the error to the paste — so it is rejected here where the fix is
    obvious. A real key starts with "AIza".
    """
    key = str(value or "").strip()
    if not key:
        return
    if key.endswith(".apps.googleusercontent.com") or key.startswith("GOCSPX-"):
        raise ValueError(
            "That is an OAuth client ID, not an API key. On Google Cloud → "
            "Credentials, use “Create credentials → API key” — the value starts "
            "with AIza."
        )
    if not key.startswith("AIza"):
        raise ValueError(
            "A Google API key starts with AIza. Check you copied the API key "
            "rather than the client ID or the search engine ID."
        )
...
def ensure_local_ai_from_ui(*, force: bool = False, model: str | None = None) -> dict[str, Any]:
    ...
    # Selecting a model has to be what makes it get used. Without this the pull
    # succeeded, the status file said "ready", and every evaluation still fell
    # back to keyword scoring because llm.provider was still "none".
    try:
        from profile_store import adopt_local_ai_provider

        adopt_local_ai_provider(str(model or status.get("model") or ""))
    except Exception as exc:
        logger.warning("Could not point the profile at Local AI: %s", exc)
    return {"ok": True, "local_ai": status}
