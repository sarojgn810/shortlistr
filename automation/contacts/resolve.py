...
def _secrets() -> dict[str, str]:
    ...
    try:
        ...
        if has_secret("SHORTLISTR_SERPER_API_KEY"):
            out["serper"] = get_secret("SHORTLISTR_SERPER_API_KEY") or ""
        if has_secret("SHORTLISTR_GITHUB_TOKEN"):
            out["github"] = get_secret("SHORTLISTR_GITHUB_TOKEN") or ""
        if has_secret("SHORTLISTR_EMAIL_VERIFY_API_KEY"):
            out["email_verify"] = get_secret("SHORTLISTR_EMAIL_VERIFY_API_KEY") or ""
    except Exception:
        pass
    try:
...
def resolve_job_contact(
    ...
    *,
    ...
    # Deduplicate by name/email; drop org logins / company slugs (not people).
    seen: set[str] = set()
    unique: list[dict[str, Any]] = []
    for p in people:
        if not _is_person_candidate(p, company):
            continue
        key = (
    ...
    for p in people:
        ...
        # Only guess addresses when we have a real first + last name.
        if email_domain and first and last:
            candidates.extend(
            ...
            for s in suggest_for_contact(
                ...
                if em and em not in {c[0] for c in candidates} and _email_looks_valid(em):
...
def _notes(
    ...
    notes: list[str] = [
        "Nothing is emailed for you — copy an address yourself, or message on LinkedIn.",
    ]
    if status == "no_person":
        notes.append(
            "No named person found in the posting. Use the LinkedIn searches below, "
            "or add someone you already know."
        )
    if status == "person_no_email":
        notes.append(
            "We found a name but no safe email guess yet. Prefer LinkedIn or the apply form."
        )
    if not secrets.get("serper"):
        notes.append(
            "Tip: add a Serper key in Connections to search the web for recruiters."
        )
    if not secrets.get("email_verify"):
        notes.append(
            "Tip: add Hunter or NeverBounce in Connections to check if an address is real."
        )
    if is_catch_all or mx_provider in ("proofpoint", "mimecast"):
        provider = (mx_provider or "their mail filter").title()
        notes.append(
            f"{provider} often accepts any address — treat guessed emails carefully."
        )
    return notes
...
def _is_person_candidate(p: dict[str, Any], company: str) -> bool:
    """Reject org logins and company-name slugs that are not real people."""
    import re

    if p.get("email") and "@" in str(p.get("email")):
        return True
    name = str(p.get("full_name") or "").strip()
    if len(name) < 3:
        return False
    # GitHub-style logins: entrupy, entrupy-interview, entrupy_build
    ...
    if company_slug and name_slug and (
        name_slug == company_slug or name_slug.startswith(company_slug)
    ):
        return False
    return True
