    ...
    *,
    ...
    for path in candidates:
        ...
        if is_ours(meta, job_id=job_id, owner=owner):
            return path, raw
        # Unstamped legacy: only if the guide embeds this job's URL.
        if not meta and url and url.rstrip("/") in raw:
            # Rewrite on next generate; still safer than company-name glob.
            if owner and owner.lower() in raw.lower():
                return path, raw
            # No owner mention — skip; user must regenerate for this profile.
            continue
    return None, None
