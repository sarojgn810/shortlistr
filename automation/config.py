...
That file is written by onboarding — `make start` on a first run, the dashboard
afterwards. `automation/setup.py` still exists for headless boxes, but it is not
the path users are pointed at.
"""
...
# ── Legacy AUTOJOB_* names ────────────────────────────────────────────────────
# The project was renamed from Autojob to Shortlistr. Anyone who installed before
# that has AUTOJOB_* in their .env, their shell, or their cron environment, and
# breaking those would silently log them out of their own LLM key. Every
# AUTOJOB_* variable is mirrored to its SHORTLISTR_* name unless that name is
# already set, so the new names win and the old ones keep working. Nothing writes
# AUTOJOB_* back — this is a one-way ramp, not a permanent alias.
def _adopt_legacy_env() -> None:
    for key, value in list(os.environ.items()):
        if key.startswith("AUTOJOB_"):
            os.environ.setdefault("SHORTLISTR_" + key[len("AUTOJOB_"):], value)
...
def _parse_filters() -> dict:
    ...
    return {
        ...
        # Matched against the job TITLE only. deal_breakers are matched against
        # title + JD body, which is right for "recruiter" or "15+ years" but
        # wrong for a role word: "manager" appears in the body of 27 of 161 real
        # postings ("report to the engineering manager") and only 6 of their
        # titles, so using deal_breakers to filter out management roles would
        # discard 21 genuine SRE jobs.
        "exclude_titles": _as_list(f.get("exclude_titles")),
    }
...
_COUNTRY_GEO: dict[str, tuple[str, ...]] = {
    ...
    # "us" itself belongs here: American listings overwhelmingly write "Remote
    # (US)" or "US Remote", and without the bare token a user who picks the
    # obvious option loses almost every one of them. Safe because
    # _hits_location_keywords word-boundary matches anything <= 3 chars, so this
    # cannot fire inside "Aarhus" — and a location field is a place, not prose.
    "us": ("united states", "usa", "u.s.", "u.s.a", "america", "us"),
    "usa": ("united states", "usa", "u.s.", "u.s.a", "america", "us"),
    "united states": ("united states", "usa", "u.s.", "u.s.a", "america", "us"),
...
EXCLUDE_TITLES = _FILTERS["exclude_titles"]
...
# The bar for "worth looking at first", reported as strong_fit on every scan.
# Deliberately well above MIN_FIT_SCORE: that one is the floor for keeping a job
# at all, so reusing it would make strong_fit equal to the number kept and say
# nothing. On a real inbox 40 marks 132 of 210 jobs and 70 marks 5.
STRONG_FIT_SCORE = int(_scoring.get("strong_fit_score", 70) or 70)
...
def reload_discovery_config() -> None:
    ...
    global MIN_SALARY_INR_LPA, MIN_SALARY_USD, DEAL_BREAKERS, EXCLUDE_TITLES
    ...
    EXCLUDE_TITLES = _as_list(filt.get("exclude_titles"))
