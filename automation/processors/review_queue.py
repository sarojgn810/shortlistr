...
# Raw-terminal single-keypress input is POSIX-only. Nothing imports this module
# — it is run directly — so guard the import rather than making Windows users
# read a ModuleNotFoundError traceback for a tool they can skip.
try:
    import termios
    import tty
...
def _getch():
    ...
    if termios is None:
        raise SystemExit(
            "The interactive reviewer needs a POSIX terminal (termios), which "
            "Windows does not provide.\nReview jobs in the dashboard instead: "
