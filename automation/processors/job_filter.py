...
def score_job(job: dict) -> dict:
    ...
    # ── 2b. Excluded titles ──────────────────────────────────────────────────
    # Title only, deliberately. deal_breakers match title + JD body, which is
    # correct for "recruiter" or "15+ years" but ruinous for a role word:
    # "manager" is in the body of 27 of 161 real postings and the title of 6, so
    # routing this through deal_breakers would bin 21 genuine SRE jobs for
    # saying "report to the engineering manager".
    for bad in (t.lower() for t in (_cfg.EXCLUDE_TITLES or []) if t):
        if bad in title:
            job["fit_score"] = 0
            job["fit_reason"] = f"excluded title: '{job.get('title', '')}'"
            return job
...
def count_strong_fit_persisted(jobs, min_score: int | None = None) -> int:
    """Strong fits counted from the database, after enrichment has run.

    Scoring a job at scan time cannot exceed 60: title match is 40, "JD not
    fetched yet" adds 10 and a preferred location adds 10, and the skill-overlap
    component — worth up to 40 — needs a job description that has not been
    fetched yet. That is why 90 jobs on a real inbox sat at exactly 60, and why
    counting strong fits from the in-memory scan list always returned zero
    however the threshold was set.

    Enrichment fetches the descriptions and re-scores, so the honest number is
    the one in the database once that has happened.
    """
    bar = _cfg.STRONG_FIT_SCORE if min_score is None else min_score
    urls = []
    for job in jobs or []:
        url = job.get("url") if isinstance(job, dict) else getattr(job, "url", None)
        if url:
            urls.append(str(url))
    if not urls:
        return 0
    ...
    ids = list({job_id_from_url(u) for u in urls})
    total = 0
    with store.db() as conn:
        # Chunked: SQLite caps host parameters just under 1000 per statement.
        for i in range(0, len(ids), 500):
            chunk = ids[i:i + 500]
            marks = ",".join("?" * len(chunk))
            row = conn.execute(
                f"SELECT COUNT(*) AS n FROM jobs "
                f"WHERE id IN ({marks}) AND COALESCE(fit_score, 0) >= ?",
                (*chunk, bar),
            ).fetchone()
...
def count_strong_fit(jobs, min_score: int | None = None) -> int:
    """How many of these clear the "look at this first" bar.

    Takes JobRecords or dicts, because discovery returns the former and the
    filter path the latter. Never re-scores: this runs on the output of a scan
    that already scored everything, and re-scoring here would be both slow and a
    second opinion nobody asked for.
    """
    bar = _cfg.STRONG_FIT_SCORE if min_score is None else min_score
    total = 0
    for job in jobs or []:
        score = job.get("fit_score") if isinstance(job, dict) else getattr(job, "fit_score", None)
        try:
            if float(score or 0) >= bar:
                total += 1
