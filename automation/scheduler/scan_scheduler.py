...
def stale_template_evals(limit: int = 500) -> list[str]:
    """Job ids whose newest evaluation fell back to the heuristic.

    A template eval is what the user gets when the AI helper was unreachable —
    a résumé/JD word overlap instead of a judgement. It is stored exactly like a
    real one, so once the outage passes there is nothing to say the score is not
    trustworthy and no way to find the affected rows. This is that query.
    """
    store.init_db()
    with store.db() as conn:
        rows = conn.execute(
            ...
            FROM eval_results e
            JOIN (
                SELECT job_id, MAX(id) AS newest FROM eval_results GROUP BY job_id
            ) latest ON latest.newest = e.id
            WHERE json_extract(e.result_json, '$.template_only') = 1
...
def reevaluate_stale(*, limit: int = 100) -> dict[str, int]:
    """Re-run evaluations that fell back to template, now the AI is reachable.

    Refuses to run when no provider is available rather than rewriting one
    heuristic result with another and resetting the timestamp — that would hide
    ...
    try:
        ...
        available = False
    if not available:
        logger.warning("Re-evaluate skipped: no AI provider available.")
        return {"candidates": len(stale_template_evals(limit)), "repaired": 0,
                "still_template": 0, "skipped_no_provider": 1}

    ids = stale_template_evals(limit)
    repaired = still_template = 0
    for jid in ids:
        try:
            with store.db() as conn:
            ...
            if getattr(result, "eval_mode", "") == "template":
                still_template += 1
            else:
                repaired += 1
        except Exception as exc:
            logger.warning("Re-evaluate %s failed: %s", jid, exc)
    logger.info("Re-evaluated %s stale template evals: %s repaired, %s still template",
                len(ids), repaired, still_template)
    return {"candidates": len(ids), "repaired": repaired,
            "still_template": still_template, "skipped_no_provider": 0}


def auto_evaluate_pending(
    *,
    limit: int = 25,
    approve_threshold: float = 0,
    ...
    ``min_fit_score`` (0–100 discovery fit) skips weak matches so we don't burn
    LLM tokens on roles the user already filtered as irrelevant.
    """
    from api.jobs_api import prepare_job_for_eval
    from eval.service import evaluate_job_text
    ...
    with store.db() as conn:
        rows = conn.execute(
            """
            SELECT j.id, j.fit_score FROM pipeline p
    ...
    for row in rows:
        ...
        try:
            fit = row["fit_score"]
            try:
                fit_n = float(fit) if fit is not None else 0.0
            except (TypeError, ValueError):
                fit_n = 0.0
            if min_fit_score > 0 and fit_n < min_fit_score:
                continue
            with store.db() as conn:
...
def run_scheduled_scan(*, tenant_id: str = "default", dry_run: bool = False) -> dict:
    ...
    if not dry_run and passed:
        ...
        try:
            ...
            stats["gmail_verify"] = verify_pending_gmail_stubs(limit=20, allow_browser=False)
        except Exception as exc:
            logger.warning("gmail verify pass failed: %s", exc)

    evaluated = 0
    auto_approved = 0
    min_score = float(settings.get("auto_evaluate_min_score") or 40)
