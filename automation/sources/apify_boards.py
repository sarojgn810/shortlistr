...
BOARD_SKIPPED: dict[str, str] = {
    ...
    # bebity/monster-jobs-scraper now 404s — the actor was removed from the store.
    # Every replacement in the store is also a paid rental, and the entry already
    # carried "0% store success historically", so this is retired rather than
    # swapped: a board that cannot run should say so in the log, not fail a scan
    # and count toward tripping the circuit breaker.
    "monster": "Actor removed from the Apify store; all replacements are paid rentals.",
}
