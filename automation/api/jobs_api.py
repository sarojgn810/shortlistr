...
_LIST_JOB_COLUMNS = """
    ...
    CASE
      WHEN LOWER(COALESCE(j.source, '')) = 'gmail'
      THEN COALESCE(json_extract(j.metadata_json, '$.verification'), 'unverified')
      ELSE COALESCE(json_extract(j.metadata_json, '$.verification'), 'confirmed')
    END AS verification,
...
def fetch_jobs(
    ...
    *,
    ...
    # relevance="all" reveals off-target finds; default Relevant hides off_target
    # / unverified Gmail only — min-fit is a client filter, not a hard hide, so
    # rescored keepers do not vanish from Discover without user action.
    ...
    if st == "inbox":
        ...
        query = f"""
            ...
            LIMIT ? OFFSET ?
        """
        params_list: list[Any] = [*_EVALUATED_PIPELINE]
        params_list.extend([limit, offset])
        params = tuple(params_list)
    else:
        query = f"""
