...
def detect_role_id(*texts: str, fallback: str | None = None) -> str:
    ...
    Returns ``fallback`` (default ``""``) when nothing matches — never invents
    Site Reliability as a silent product default.
    """
...
def role_from_profile() -> str:
    """LinkedIn target role from live profile.yml titles only.

    Empty / unmatched titles → "" (UI shows "Choose a target role"). Never
    fall through to discovery SEARCH_KEYWORDS or a hardcoded SRE default —
    those made every fresh onboarding look like an SRE setup.
    """
    titles = _profile_target_titles()
    if titles:
        return detect_role_id(*titles, fallback="")
    return ""


def _profile_target_titles() -> list[str]:
    """Read targeting titles from profile.yml (fresh), then in-memory config."""
    titles: list[str] = []
    try:
        import os
        ...
        if os.path.isfile(PROFILE_PATH):
            ...
            if isinstance(data, dict):
                ...
                raw = filters.get("target_titles") or []
    ...
    try:
        ...
        raw = list(filters.get("target_titles") or [])
        return [str(t).strip() for t in raw if str(t).strip()]
    except Exception:
        return []


def profile_titles_for_ui() -> list[str]:
    """Titles shown under the LinkedIn Target role control."""
    return _profile_target_titles()[:12]


# Used when callers need a pack but the user has not chosen a role yet.
_EMPTY_ROLE: dict = {
    "id": "",
    ...
    "seniority": [],
    "impact_verbs": [],
    "headline_template": "{seniority}Engineer",
    "about_beats": [
        "Pick a target role above so suggestions match how recruiters search.",
    ],
}
...
def get_role(role_id: str | None = None) -> dict:
    ...
    if key in ROLE_PACKS:
        pack = dict(ROLE_PACKS[key])
        # Prefer the user's own targeting titles in Open to work / search lists.
        profile_titles = _profile_target_titles()
        if profile_titles:
            merged: list[str] = []
            seen: set[str] = set()
            for t in profile_titles + list(pack.get("search_titles") or []):
                low = t.lower()
                if low in seen:
                    continue
                seen.add(low)
    ...
    if fallback in ROLE_PACKS:
        return get_role(fallback)
    return dict(_EMPTY_ROLE)
