...
from linkedin_optimizer.parser import looks_like_location, profile_from_structured
...
def profile_from_cv_markdown(md: str) -> dict[str, Any]:
    ...
    # Same rule as the LinkedIn paste path: a contact row is a mix of email,
    # phone, links and — sometimes — a place. "has letters and is short" also
    # matches a job title, which is how the headline ended up in the location.
    location = ""
    for part in re.split(r"[•·|]", contact):
        p = part.strip()
        if looks_like_location(p):
            location = re.sub(r"[*_`]+", "", p).strip()
            break
