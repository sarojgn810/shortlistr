...
def _remove_crons() -> list[str]:
    ...
    # Legacy marker from setup_cron.sh
    try:
        current = subprocess.check_output(["crontab", "-l"], stderr=subprocess.DEVNULL, text=True)
    except (subprocess.CalledProcessError, FileNotFoundError, OSError):
        return actions
    markers = ("# shortlistr-referral-engine", "# shortlistr", "automation.cli ingest", "run_daily.py")
    if not any(m in current for m in markers):
        return actions
    kept = [
        ln
        for ln in current.splitlines()
        if "# shortlistr-referral-engine" not in ln
...
def _purge_user_data() -> list[str]:
    ...
    for d in (DATA_DIR, OUTPUT_DIR, os.path.join(SHORTLISTR_ROOT, "reports"),
...
def _purge_build_artifacts() -> list[str]:
    ...
    # Python caches under automation/
