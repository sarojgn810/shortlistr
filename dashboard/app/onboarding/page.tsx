...
import { api, ApiError, type AtsScore, type AutomationSettings, type CvTemplate, type ConnectionsSetup, type ProfileSetup, type SetupStatus } from "@/src/lib/api/client";
import { ArrowRight, CheckCircle2, FileText } from "lucide-react";
import { ConnectStep } from "@/src/components/onboarding/ConnectStep";

const STEPS = ["Welcome", "Profile", "Resume", "Template", "Connect", "Review", "Done"] as const;
...
export default function OnboardingPage() {
  ...
  const [connections, setConnections] = useState<ConnectionsSetup | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [profileData, setup, conns] = await Promise.all([
      ...
      <div className="w-full space-y-8">
        ...
        {step === 4 && (
          <ConnectStep
            ...
            llmKeySet={Boolean(setupStatus?.llm?.api_key_set)}
            onSaveLlmKey={async (key) => {
              await api.saveProfile({ llm_api_key: key });
            }}
            onSaved={async () => {
