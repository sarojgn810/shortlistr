...
function displayScore(job: Job): string {
  // Eval is already 0–5. Discovery fit_score is 0–100; map with /20 so a
  // typical title+location hit (~60) reads as ~3.0/5, never above 5.
  if (job.eval_score != null && job.eval_score > 0) {
    return `${Math.min(5, job.eval_score).toFixed(1)}/5`;
  }
  if (job.fit_score > 0) {
    return `${Math.min(5, job.fit_score / 20).toFixed(1)}/5`;
  }
  return "—";
}
