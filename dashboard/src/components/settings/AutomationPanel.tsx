  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <p className="rounded-xl bg-mist/40 px-4 py-3 text-sm text-stone">
        ...
        {typeof settings.last_scan_sources_ok === "number" &&
        typeof settings.last_scan_sources_total === "number" &&
        settings.last_scan_sources_total > 0
          ? ` · ${settings.last_scan_sources_ok}/${settings.last_scan_sources_total} sources ok`
          : ""}
        {settings.last_scan_jobs != null ? ` · ${settings.last_scan_jobs} new jobs` : ""}
      </p>

      {settings.last_scan_errors &&
        Object.keys(settings.last_scan_errors).length > 0 && (
          <div className="rounded-xl border border-orange/30 bg-orange/10 px-4 py-3 text-sm text-ink">
            <p className="font-bold">Some sources failed last scan</p>
            <ul className="mt-2 list-disc space-y-1 pl-4 text-stone">
              {Object.entries(settings.last_scan_errors).map(([src, err]) => (
                <li key={src}>
                  <span className="font-semibold text-ink">{src}</span>
                  {": "}
                  {String(err).replace(/\s+/g, " ").slice(0, 180)}
                  {String(err).length > 180 ? "…" : ""}
                </li>
              ))}
            </ul>
          </div>
