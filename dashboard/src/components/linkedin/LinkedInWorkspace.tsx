  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-[28px] border border-mist bg-white/80 p-4 backdrop-blur">
        ...
        {(state?.profile_target_titles || []).length > 0 ? (
          <p className="basis-full text-xs text-stone sm:basis-auto sm:max-w-md">
            From profile targeting:{" "}
            <span className="font-semibold text-ink">
              {(state?.profile_target_titles || []).slice(0, 4).join(", ")}
              {(state?.profile_target_titles || []).length > 4 ? "…" : ""}
            </span>
          </p>
        ) : (
          <p className="basis-full text-xs text-stone sm:basis-auto">
            Set target titles on Profile to auto-select this role.
          </p>
        )}
