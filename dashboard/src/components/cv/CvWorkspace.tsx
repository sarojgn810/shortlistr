...
export function CvWorkspace({
  ...
  const loadPreview = useCallback(
    async (tpl: string, md?: string, preferSinglePage?: boolean) => {
      ...
      try {
        ...
        const preferSingle = preferSinglePage ?? pageTarget === "1";
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (content.markdown?.trim()) {
        // Inline preview — do not call loadPreview here. That callback depends on
        // pageTarget; wiring it into refresh recreated this effect and snapped
        // Length toggles back to the server value.
        setPreviewLoading(true);
        try {
          const tpl = status.cv_settings?.template_id || "ats-single";
          const preferSingle = (status.cv_settings?.page_target || "auto") === "1";
          const res = await api.previewCv(tpl, content.markdown, false, preferSingle);
          setPreviewHtml(res.html);
        } catch {
          setPreviewHtml(null);
        } finally {
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-base text-stone">
                Choose your original PDF or an Shortlistr template compiled for ATS parsers.
              </p>
            </div>
          ...
          <div className="order-1 lg:order-2 lg:col-span-3">
            ...
            >
              ...
              <CvHtmlPreview
                ...
                allowMultiPage={pageTarget !== "1"}
                emptyMessage="Pick an Shortlistr template to preview your resume."
              />
