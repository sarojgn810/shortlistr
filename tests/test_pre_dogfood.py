...
def test_relevance_filter_keeps_low_fit_relevant_jobs_visible(isolated):
    """Min fit is a client-side filter, not a server-side hide.

    `fetch_jobs` used to drop relevant-but-low-fit rows below the profile's
    min_fit_score. That made Discover unstable: a rescore could silently
    delete a job the user was still looking at. The gate now hides only
    off_target / unverified finds, and the dashboard applies min_fit_score
    itself, so a rescored keeper never vanishes without user action.
    """
    from api.jobs_api import fetch_jobs
    from store import db as store
    ...
    assert {strong_id, weak_id} <= default_ids
