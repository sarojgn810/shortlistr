...
def test_chat_llm_error_falls_back_to_commands(monkeypatch):
    """Misconfigured Groq model must not leave the user with a raw exception."""
    _isolate(monkeypatch)
    import llm
    from store import db
    ...
    class _Boom:
        ...
        def complete(self, prompt, system="", max_tokens=1024):
            raise RuntimeError("model_not_found: qwen2.5:0.5b")

    db.init_db()
    monkeypatch.setattr(llm, "get_llm", lambda: _Boom())
    from agent.chat import chat

    out = chat("status")
    assert "LLM error" not in out["reply"]
    assert out["actions"] and out["actions"][0]["tool"] == "shortlistr.status"
    assert out.get("needs_llm") is True
