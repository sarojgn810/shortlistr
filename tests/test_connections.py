...
def test_google_cse_credentials_via_connections(isolated):
    from connections_store import get_connections_for_ui, save_connections_from_ui
    from secrets_store import has_secret

    before = get_connections_for_ui()["google_cse"]
    assert before["ready"] is False

    save_connections_from_ui(
        {"google_cse_api_key": "AIza-test", "google_cse_cx": "cx123abc"}
    )
    data = get_connections_for_ui()["google_cse"]
    assert data["api_key_set"] is True
    assert data["cx_set"] is True
    assert data["ready"] is True
    assert has_secret("GOOGLE_CSE_API_KEY")
    assert has_secret("GOOGLE_CSE_CX")

    save_connections_from_ui({"google_cse_api_key": "", "google_cse_cx": ""})
    after = get_connections_for_ui()["google_cse"]
    assert after["ready"] is False
...
def test_connections_api_page_reader_enable(isolated, monkeypatch):
    """Regression: ConnectionsBody must accept page_reader_enabled."""
    ...
    monkeypatch.setattr(paths, "PROFILE_PATH", str(isolated["profile"]))
    ...
    assert before["page_reader"]["enabled"] is False

    after = client.put(
        "/setup/connections",
        json={"page_reader_enabled": True},
    ).json()
    assert after["page_reader"]["enabled"] is True
    assert after["page_reader"]["ready"] is True

    import yaml

    data = yaml.safe_load(isolated["profile"].read_text(encoding="utf-8"))
    assert data["sources"]["page_reader"]["enabled"] is True
    assert "agent_reach" not in data.get("sources", {})
