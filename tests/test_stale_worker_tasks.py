...
# ── Heartbeat liveness (v18) ─────────────────────────────────────────────────
#
# The 30-minute timer above was a mitigation, not a fix. The root cause is that
# the scan runs in a daemon thread: Python kills those outright when the process
# exits, so `_finish` never runs. `make api` runs under --reload, so any saved
# file restarts the server, and a scan takes minutes — the window is wide open.
# A live worker now beats every HEARTBEAT_SECONDS, so silence is proof it died.


def _insert_running_with_heartbeat(conn, *, beat_seconds_ago: int) -> int:
    cur = conn.execute(
        """
        INSERT INTO worker_queue
            (task_type, payload_json, status, created_at, started_at, heartbeat_at, owner)
        VALUES ('discover', '{}', 'running', datetime('now'), datetime('now'),
                datetime('now', ?), 'somehost:999999')
        """,
        (f"-{beat_seconds_ago} seconds",),
    )
    return int(cur.lastrowid)


def test_worker_queue_tracks_owner_and_heartbeat(isolated_data_dir):
    from store import db as store

    store.init_db()
    with store.db() as conn:
        cols = {r["name"] for r in conn.execute("PRAGMA table_info(worker_queue)")}
    assert {"owner", "heartbeat_at"} <= cols


def test_a_silent_worker_is_reaped_without_waiting_for_the_timeout(isolated_data_dir):
    """The whole point: a killed worker frees its row in ~a minute, not 30."""
    from store import db as store
    from store.db import HEARTBEAT_STALE_SECONDS, reap_stale_tasks

    store.init_db()
    with store.db() as conn:
        _insert_running_with_heartbeat(conn, beat_seconds_ago=HEARTBEAT_STALE_SECONDS + 10)
        assert reap_stale_tasks(conn) == 1
        assert _statuses(conn) == {"failed": 1}


def test_a_beating_worker_is_never_reaped(isolated_data_dir):
    """A scan legitimately runs for minutes — killing it mid-run costs a rescan."""
    from store import db as store
    from store.db import STALE_TASK_MINUTES, reap_stale_tasks

    store.init_db()
    with store.db() as conn:
        # Claimed long ago by the old timer's standard, but still beating.
        cur = conn.execute(
            """
            INSERT INTO worker_queue
                (task_type, payload_json, status, created_at, started_at, heartbeat_at, owner)
            VALUES ('discover', '{}', 'running', datetime('now', ?), datetime('now', ?),
                    datetime('now'), 'somehost:1')
            """,
            (f"-{STALE_TASK_MINUTES + 30} minutes", f"-{STALE_TASK_MINUTES + 30} minutes"),
        )
        assert cur.lastrowid
        assert reap_stale_tasks(conn) == 0, "a beating worker was killed mid-scan"
        assert _statuses(conn) == {"running": 1}


def test_a_dead_worker_stops_blocking_the_next_scan(isolated_data_dir):
    """End to end: the Scan click after a crash must actually queue and run."""
    from store import db as store
    from store.db import HEARTBEAT_STALE_SECONDS
    from workers.discovery_worker import _claim_pending

    store.init_db()
    with store.db() as conn:
        dead = _insert_running_with_heartbeat(
            conn, beat_seconds_ago=HEARTBEAT_STALE_SECONDS + 10
        )

    new_id = store.enqueue_task("discover", {"dry_run": True})
    assert new_id != dead, "the Scan click was answered with the dead row's id"
    assert [c["id"] for c in _claim_pending()] == [new_id], "new scan was refused"


def test_claiming_stamps_owner_and_first_beat(isolated_data_dir):
    """A task must not look stale the instant it is claimed."""
    from store import db as store
    from workers.discovery_worker import _claim_pending

    store.init_db()
    store.enqueue_task("discover", {"dry_run": True})
    claimed = _claim_pending()
    assert len(claimed) == 1

    with store.db() as conn:
        row = conn.execute(
            "SELECT owner, heartbeat_at FROM worker_queue WHERE id = ?",
            (claimed[0]["id"],),
        ).fetchone()
    assert row["heartbeat_at"], "claim must set the first heartbeat"
    assert row["owner"] == store.WORKER_ID


def test_heartbeat_only_touches_a_running_task(isolated_data_dir):
    """A finished task must not be resurrected into looking alive."""
    from store import db as store

    store.init_db()
    tid = store.enqueue_task("discover", {"dry_run": True})
    with store.db() as conn:
        conn.execute("UPDATE worker_queue SET status='done' WHERE id=?", (tid,))

    store.touch_task_heartbeat(tid)
    with store.db() as conn:
        row = conn.execute(
            "SELECT status, heartbeat_at FROM worker_queue WHERE id=?", (tid,)
        ).fetchone()
