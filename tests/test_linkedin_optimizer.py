...
def test_normalize_linkedin_url():
    assert normalize_linkedin_url("linkedin.com/in/janesmith") == (
        "https://www.linkedin.com/in/janesmith"
    )
    assert normalize_linkedin_url("janesmith") == "https://www.linkedin.com/in/janesmith"
...
def test_role_from_profile_titles(monkeypatch, tmp_path):
    import config
    import paths
    ...
    monkeypatch.setattr(config, "_FILTERS", {"target_titles": []}, raising=False)
...
def test_role_from_profile_empty_never_defaults_to_sre(monkeypatch, tmp_path):
    """Fresh onboarding / empty titles must not pre-select Site Reliability."""
    import config
    import paths
    from linkedin_optimizer.roles import get_role, role_from_profile
    ...
    assert role_from_profile() == ""
    assert get_role(None)["id"] == ""
    assert get_role("")["label"].lower().startswith("choose")
...
def test_stale_linkedin_draft_ignored(tmp_path, monkeypatch):
    ...
    # Profile.yml is what role_from_profile reads first.
    (tmp_path / "config").mkdir(parents=True, exist_ok=True)
    (tmp_path / "config" / "profile.yml").write_text(
    ...
    (tmp_path / "li.json").write_text(
        '{"owner":"previous@example.com","target_role":"sre",'
        '"profile":{"name":"Previous Owner","headline":"SRE","about":"'
        + ("x" * 100)
    ...
    assert state["profile_target_titles"][:1] == ["Full Stack Developer"]
    assert (state.get("profile") or {}).get("name") in ("", "Other Person")


def test_profile_titles_override_draft_role(tmp_path, monkeypatch):
    """Live profile targeting wins over a saved LinkedIn draft role."""
    import config
    import linkedin_optimizer.service as svc
    import paths
    from linkedin_optimizer.service import get_state
    ...
    (tmp_path / "li.json").write_text(
        '{"target_role":"devops","profile":{"name":"Ada","headline":"DevOps",'
        '"about":"' + ("y" * 100) + '","experience":[{"title":"DevOps","bullets":["a","b"]}],'
        '"skills":["aws","k8s","terraform","docker","linux"]}}',
        encoding="utf-8",
    )
    state = get_state()
    assert state["target_role"] == "sre"
    assert "Site Reliability Engineer" in state["profile_target_titles"]
